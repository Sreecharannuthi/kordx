...
import com.android.rockages.kordx.services.groove.PlaylistParser
...
class PlaylistRepository(private val kordx: KordX) {
 enum class SortBy {
 CUSTOM,
 TITLE,
 TRACKS_COUNT,
 ...
 val playlists = kordx.database.playlists.entries()
 playlists.values.map { x ->
 val playlist = when {
 x.isLocal -> {
 val uri = x.uri!!
 ActivityUtils.makePersistableReadableUri(context, uri)
 PlaylistParser.parse(kordx, x.id, uri)
 }
 ...
 id = id,
 title = title,
 songPaths = songIds.mapNotNull { kordx.groove.song.get(it)?.path },
 uri = null,
 path = null,
 )
 ...
 kordx.database.playlists.insert(playlist)
 }
 }
 ...
 runCatching {
 ActivityUtils.makePersistableReadableUri(kordx.applicationContext, it)
 }
 }
 _all.update {
 it - id
 ...
 emitCount()
 kordx.groove.coroutineScope.launch {
 kordx.database.playlists.delete(id)
 }
 }
 ...
 kordx.groove.coroutineScope.launch {
 kordx.database.playlists.update(updated)
 }
 }
 ...
 val songIds = favorites.getSongIds(kordx)
 ...
 val outputStream = kordx.applicationContext.contentResolver.openOutputStream(uri, "w")
 outputStream?.use {
 ...
 emitUpdateId()
 kordx.groove.coroutineScope.launch {
 kordx.database.playlists.update(renamed)
 }
 }
