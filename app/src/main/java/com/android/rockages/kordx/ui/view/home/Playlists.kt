...
fun PlaylistsView(context: ViewContext) {
 val isUpdating by context.kordx.groove.playlist.isUpdating.collectAsState()
 val playlists by context.kordx.groove.playlist.all.collectAsState()
 val playlistsCount by context.kordx.groove.playlist.count.collectAsState()
 ...
 uris.forEach { x ->
 try {
 ActivityUtils.makePersistableReadableUri(context.kordx.applicationContext, x)
 val playlist = PlaylistParser.parse(context.kordx, null, x)
 ...
 context.kordx.t.InvalidM3UFile,
 Toast.LENGTH_LONG,
 ).show()
 }
 }
...
private fun PlaylistControlBar(
 ...
 Text(context.kordx.t.NewPlaylist)
 }
 }
 ElevatedButton(
 modifier = Modifier.weight(1f),
 ...
 Spacer(modifier = Modifier.width(8.dp))
 Text(context.kordx.t.ImportPlaylist)
 }
 }
 }
}
