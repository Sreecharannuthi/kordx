...
class Radio(private val kordx: KordX) : KordX.Hooks, RadioAdapterTarget {
    ...
    // The override uses the concrete [RadioQueue] / [RadioShorty]
    // types (which implement [RadioQueueAdapterTarget] /
    // [RadioShortyAdapterTarget]) so existing call sites that
    // depend on the concrete types — `radio.queue.add(...)`,
    // `radio.shorty.previous()` — keep compiling. Kotlin permits
    // covariant override of `val` getters.
    ...
    private var mediaServiceUpdateSubscription: EventUnsubscribeFn? = null
    ...
    private val exoPlayerListener = object : Player.Listener {
        override fun onMediaItemTransition(mediaItem: MediaItem?, reason: Int) {
            // Re-arm the wake lock on every track start (it is acquired
            // without a timeout, but this covers any path that released it
            // while playback continues with the next item). Guarded by
            // playWhenReady: transitions also fire during paused queue
            // syncs, where the lock must stay off.
            if (exoPlayer.playWhenReady) acquireWakeLock()
    ...
    private fun acquireWakeLock() {
        try {
            // No timeout: the 10-minute variant silently dropped the lock
            // mid-long-track. Released explicitly in pause()/stop()/destroy()
            // and re-armed on every media-item transition.
    ...
    fun ready() {
        ...
        // Issue #773 / #707: restore the persisted loop mode and shuffle mode
        // on app start. The code held these in memory only, so the user's
        // "Repeat all" / "Shuffle on" choice was lost on every app restart.
        // The settings are read here (before `attachGrooveListener` so the
        // order is deterministic), then the queue's `setLoopMode` /
        // `setShuffleMode` are called with `persist = false` semantics by way
        // of the `if (existing != value) persist` guard inside those methods
        // (a noop write doesn't repersist the same value).
        ...
        session.attachDebugReceivers()
        attachMediaServiceUpdateSubscriber()
        observatory.start()
    }
    ...
    fun destroy() {
        ...
        releaseWakeLock()
        observatory.destroy()
        mediaServiceUpdateSubscription?.invoke()
        mediaServiceUpdateSubscription = null
        session.detachDebugReceivers()
    ...
    fun play(options: PlayOptions) {
        ...
        while (true) {
            val targetId = queue.getSongIdAt(index)
            val targetSong = targetId?.let { kordx.groove.song.get(it) }
            if (targetSong == null) {
                // Stale id: drop it and try the next candidate when autostarting.
                ...
                index = options.index.coerceAtMost(queue.currentQueue.size - 1)
                continue
            }
    ...
    /**
     * Start the Media3 [KordXMediaLibraryService] as a foreground service when playback
    ...
    private fun startMediaLibraryService() {
        ...
        ctx.startForegroundService(Intent(ctx, KordXMediaLibraryService::class.java))
    }

    private fun attachMediaServiceUpdateSubscriber() {
        mediaServiceUpdateSubscription = onUpdate.subscribe { event ->
            when (event) {
                is Events.Player.Started,
                is Events.Player.Resumed -> startMediaLibraryService()
                else -> Unit
            }
            if (event is Events.Player.Started) {
                val songId = queue.currentSongId
                if (songId != null) {
                    kordx.groove.recentPlays.add(songId)
                    Logger.warn("Radio", "recentPlays.add: songId=$songId")
                }
            }
        }
    }
