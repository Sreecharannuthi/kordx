...
fun Song.toSamplingInfoString(kordx: KordX): String? {
    ...
    // The fix uses `listOfNotNull(...)` (declarative, no mutable
    // accumulator) AND wraps each `?.let { ... }` in `.takeIf {
    // it.isNotBlank() }` so any empty / blank formatted string is
    // also filtered out. The result is a stable `Lavf57.82.101`-only
    // string for the FLAC case (instead of `Lavf57.82.101, , ,`).
