...
fun AddToPlaylistDialog(
 ...
 val allPlaylistsIds by context.kordx.groove.playlist.all.collectAsState()
 val playlists by remember(allPlaylistsIds) {
 derivedStateOf {
 allPlaylistsIds
 .mapNotNull { context.kordx.groove.playlist.get(it) }
 ...
 content = {
 when {
 playlists.isEmpty() -> SubtleCaptionText(context.kordx.t.NoInAppPlaylistsFound)
 else -> LazyColumn(modifier = Modifier.padding(bottom = 4.dp)) {
 items(playlists) { playlist ->
 val playlistSongIds = playlist.getSongIds(context.kordx)
