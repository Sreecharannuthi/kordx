...
class MainActivityMediaSearchForwardingTest {
    ...
    private fun loadSource(relativePath: String): String {
        ...
        // Resolve relative to the project root. The gradle test
        // classpath runs from the project root (./gradlew :app:test).
    ...
    fun mainActivityOnCreateForwardsTheLaunchIntent() {
        ...
        // so a cold-start alias launch (e.g. the AVD validation step
        // `am start -a android.intent.action.MEDIA_PLAY_FROM_SEARCH --es query "Moth"`)
        // actually reaches the radio session.
        assertTrue(
    ...
    fun mainActivityOnCreateForwardingIsAfterEmitActivityReady() {
        ...
        // The forwarding call must come AFTER `kordx.emitActivityReady()`
        // so `KordX.instance` is populated when the forward runs (the
        // forward reads `KordX.instance` via the static `Volatile`
        // field; a forward before `emitActivityReady` would hit the
        // null-instance defensive return and drop the search).
    ...
    fun mainActivityOverridesOnNewIntent() {
        ...
        // The override must exist with the right signature
        // (Android's `ComponentActivity.onNewIntent(intent: Intent)`).
        assertTrue(
    ...
    fun mainActivityOnNewIntentSetsTheIntent() {
        ...
        // The `setIntent(intent)` call must be present in `onNewIntent`
        // so any subsequent `getIntent()` call (e.g. from a composable
        // that inspects the launch intent) sees the new intent, not the
        // original cold-start one.
    ...
    fun mainActivityOnNewIntentForwardsTheNewIntent() {
        ...
        // The `forwardMediaSearchIntent(intent)` call must be present
        // in `onNewIntent` so warm-start alias launches retrigger
        // the forwarding.
        val onNewIntentStart = source.indexOf("override fun onNewIntent(intent: Intent)")
        assertTrue(onNewIntentStart >= 0, "onNewIntent override not found")
        val body = source.substring(onNewIntentStart)
    ...
    fun mainActivityHelperExists() {
        ...
        // The helper must exist (private — not exposed in the public
        // surface; the call sites are `onCreate` and `onNewIntent`).
        assertTrue(
    ...
    fun mainActivityHelperFiltersOnMediaPlayFromSearchAction() {
        ...
        // The helper must early-return for any non-MEDIA_PLAY_FROM_SEARCH
        // action so the standard `MAIN` / `LAUNCHER` launch intent and
        // any other intents are not touched. We assert both the action
        // check is present and the right constant is used (the public
        // `MediaStore.INTENT_ACTION_MEDIA_PLAY_FROM_SEARCH` constant,
        // not a hardcoded string — keeps the test aligned with the
        // SDK's canonical action string).
        ...
        // a `return` (the early-return for non-matching actions).
    ...
    fun mainActivityHelperCallsHandlePlayFromSearchOnRadioSession() {
        ...
        // The helper must forward to
        // `KordX.instance.radio.session.handlePlayFromSearch(query)`.
        // The static `KordX.instance` access is the live-instance
    ...
    fun mainActivityHelperToleratesNullKordXInstance() {
        ...
        // The helper must tolerate a `null` `KordX.instance` (e.g.
        // an `am start` race before `KordX` finishes initializing)
        // by logging + returning without crashing. The activity is
        // the `singleTask` root, so a crash here would black-hole
        // the framework; the defensive return is intentional.
        val helperStart = source.indexOf("private fun forwardMediaSearchIntent(intent: Intent?)")
        assertTrue(helperStart >= 0, "forwardMediaSearchIntent helper not found")
        val body = source.substring(helperStart)
        ...
        // The null-instance path must log (so the AVD validation gate
        // can confirm the defensive return fired if KordX is ever
        // null at the right time).
    ...
    fun mainActivityHelperLogsTheForwardedQuery() {
        ...
        // The helper must log the forwarded query so the AVD
        // validation gate (`adb logcat -d | grep "MainActivity.*MEDIA_PLAY_FROM_SEARCH"`)
        // can confirm the forwarding fired.
        val helperStart = source.indexOf("private fun forwardMediaSearchIntent(intent: Intent?)")
        assertTrue(helperStart >= 0, "forwardMediaSearchIntent helper not found")
        val body = source.substring(helperStart)

        // Whitespace-tolerant: a regex that matches `Logger.warn(`
        // followed by any whitespace, then `"MainActivity"`.
        assertTrue(
        ...
        // The log must include the literal `MEDIA_PLAY_FROM_SEARCH`
        // tag so the AVD logcat grep is unambiguous.
        assertTrue(
    ...
    @Test
    fun mainActivityDoesNotShadowTheRadioSessionHandler() {

        // Regression guard: the forwarding is the routing surface for the
        // `MEDIA_PLAY_FROM_SEARCH` activity-alias. The Media3 browse-tree
        // search path (used when the user is actively in the Auto UI) lives
        // in [KordXMediaLibraryService] and converges on the same fuzzy
        // search / play-queue logic; this test only pins that the alias
        // path keeps using [RadioSession.handlePlayFromSearch] rather than
        // introducing a duplicate handler in MainActivity.
        ...
        assertTrue(
            ...
            "RadioSession's `handlePlayFromSearch(query: String?)` must remain " +
                "intact — it's the shared routing target for the alias path."
        )

        // The MainActivity forwarding must NOT introduce a duplicate
        // `handlePlayFromSearch` definition in MainActivity.kt
        // (the routing must stay in RadioSession, not in MainActivity).
        ...
        assertFalse(
            ...
            "MainActivity must NOT define a `handlePlayFromSearch` " +
                "function — the routing must stay in RadioSession."
        )
    }
    ...
    fun mainActivityImportForMediaSearchExtrasIsPresent() {
        ...
        // The `android.provider.MediaStore` import must be present
        // so the `MediaStore.INTENT_ACTION_MEDIA_PLAY_FROM_SEARCH` /
        // `MediaStore.EXTRA_MEDIA_FOCUS` / `MediaStore.EXTRA_MEDIA_TITLE`
        // references resolve.
