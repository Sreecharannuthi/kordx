 ...
 */
 ...
 // The autostart == true path now recovers by iterating to the next candidate.
 assertTrue(
 nullSongBlock.contains("queue.removeAtSilently(index)"),
 "`if (targetSong == null)` block must drop the stale id via removeAtSilently for the autostart == true path"
 )
 assertTrue(
 nullSongBlock.contains("index = options.index.coerceAtMost"),
 "`if (targetSong == null)` block must advance the iterator for the autostart == true path"
 )
 }
 ...
 val autostartIdx = playBody.indexOf("!options.autostart")
 val staleCleanupIdx = playBody.indexOf("queue.removeAtSilently(index)")
 assertTrue(autostartIdx >= 0, "autostart check not found")
 assertTrue(staleCleanupIdx >= 0, "stale-id cleanup not found")
 assertTrue(
 autostartIdx < staleCleanupIdx,
 "The early-return guard (`!options.autostart`) must precede the stale-id cleanup " +
 "inside the `if (targetSong == null)` block, so the loop is short-circuited."
 )
 }
 ...
 assertTrue(staleCleanupIdx >= 0, "stale-id cleanup not found in Radio.play()")
 assertTrue(
 guardIdx < staleCleanupIdx,
 "The early-return guard must appear BEFORE the stale-id cleanup in the source."
 )
 }
 ...
 assertTrue(
 playBody.contains("playAtIndex("),
 "Radio.play() should dispatch to `playAtIndex()` to prepare the ExoPlayer playlist"
 )
 assertTrue(
 playBody.contains("queue.getSongIdAt(index)"),
 "Radio.play() should resolve the song id from the queue using a mutable iterator index"
 )
 assertTrue(
